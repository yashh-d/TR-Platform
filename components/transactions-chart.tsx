...
const transactionMetrics = [
  { id: "txCount", label: "Daily Transactions" },
  { id: "cumulativeTxCount", label: "Cumulative Transactions" }
]

export function TransactionsChart({ network }: TransactionsChartProps) {
  const [metricsData, setMetricsData] = useState<any[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [timeRange, setTimeRange] = useState<"7D" | "30D" | "3M" | "6M" | "1Y" | "ALL">("30D")
  const [activeMetric, setActiveMetric] = useState<string>("txCount")
  ...
  useEffect(() => {
    async function fetchMetricsData() {
      try {
        ...
        const selectColumns = ['date', 'txCount', 'cumulativeTxCount']
  ...
  function updateChartForMetric(metricName: string, data: any[], range: string, network: string) {
    ...
    const metricInfo = transactionMetrics.find(m => m.id === metricName)
  ...
  if (loading) return (
    <div className="h-[350px] flex items-center justify-center">
      <div className="animate-pulse text-gray-500">Loading transaction data...</div>
    </div>
  )
