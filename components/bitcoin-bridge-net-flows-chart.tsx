...
export function BitcoinBridgeNetFlowsChart() {
  ...
  const fetchBridgeData = async () => {
    try {
      ...
      while (hasMore) {
        console.log(`[BitcoinBridgeNetFlowsChart] Fetching page ${page + 1}...`)
      ...
      console.log(`[BitcoinBridgeNetFlowsChart] Total transactions fetched: ${allTransactions.length}`)
      ...
      // Process transactions to calculate daily net flows
  ...
  if (loading) {
    return (
      <div className="p-6 border rounded-lg">
        <h3 className="text-lg font-semibold mb-4">Daily Net Flows (Mints - Burns)</h3>
  ...
  const plotData = [
    {
      ...
      y: filteredData.map(d => d.netFlow),
      type: 'bar' as const,
      name: 'Net Flow',
      marker: {
        color: filteredData.map(d => d.netFlow >= 0 ? '#10B981' : '#EF4444'), // Green for positive, red for negative
      },
      hovertemplate: '<b>%{x}</b><br>Net Flow: %{y:.4f} BTC<extra></extra>'
    }
  ]
  ...
  const totalNetFlow = filteredData.reduce((sum, d) => sum + d.netFlow, 0)
  const positiveFlows = filteredData.filter(d => d.netFlow > 0).length
  const negativeFlows = filteredData.filter(d => d.netFlow < 0).length

  return (
    <div className="p-6 border rounded-lg">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold">Daily Net Flows (Mints - Burns)</h3>
