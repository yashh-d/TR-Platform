...
interface ContractsDeployersChartProps {
  network: string
}

// Contracts & Deployers specific metrics
const contractsDeployersMetrics = [
  { id: "cumulativeContracts", label: "Cumulative Contracts" },
  { id: "cumulativeDeployers", label: "Cumulative Deployers" }
]

export function ContractsDeployersChart({ network }: ContractsDeployersChartProps) {
  const [metricsData, setMetricsData] = useState<any[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [timeRange, setTimeRange] = useState<"7D" | "30D" | "3M" | "6M" | "1Y" | "ALL">("30D")
  const [activeMetric, setActiveMetric] = useState<string>("cumulativeContracts")
  ...
  useEffect(() => {
    async function fetchMetricsData() {
      try {
        ...
        const selectColumns = ['date', 'cumulativeContracts', 'cumulativeDeployers']
        ...
        console.error('Error fetching contracts/deployers data:', err)
  ...
  function updateChartForMetric(metricName: string, data: any[], range: string, network: string) {
    ...
    const metricInfo = contractsDeployersMetrics.find(m => m.id === metricName)
  ...
  if (loading) return (
    <div className="h-[350px] flex items-center justify-center">
      <div className="animate-pulse text-gray-500">Loading contracts & deployers data...</div>
    </div>
  )
  ...
  if (!metricsData || metricsData.length === 0) return (
    <div className="h-[350px] flex items-center justify-center">
      <div className="text-gray-500">No contracts & deployers data available</div>
    </div>
  )
  ...
  return (
    <div className="border rounded-md bg-white p-4">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-bold">Contracts + Deployers</h2>
      </div>
      
      {/* Metric Pills */}
      <div className="mb-4">
        <div className="flex space-x-2">
          {contractsDeployersMetrics.map(metric => (
            <Button
              key={metric.id}
