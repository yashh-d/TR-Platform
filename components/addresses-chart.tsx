...
export function AddressesChart({ network }: AddressesChartProps) {
  const [metricsData, setMetricsData] = useState<any[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [timeRange, setTimeRange] = useState<"7D" | "30D" | "3M" | "6M" | "1Y" | "ALL">("30D")
  const [activeMetric, setActiveMetric] = useState<string>("activeAddresses")
  ...
  useEffect(() => {
    async function fetchMetricsData() {
      try {
        ...
        const selectColumns = ['date', 'activeAddresses', 'cumulativeAddresses']
  ...
  function updateChartForMetric(metricName: string, data: any[], range: string, network: string) {
    ...
    const metricInfo = addressMetrics.find(m => m.id === metricName)
  ...
  if (loading) return (
    <div className="h-[350px] flex items-center justify-center">
      <div className="animate-pulse text-gray-500">Loading address data...</div>
    </div>
  )
  ...
  if (!metricsData || metricsData.length === 0) return (
    <div className="h-[350px] flex items-center justify-center">
      <div className="text-gray-500">No address data available</div>
    </div>
  )
