...
export function StablecoinMetricsChart({ 
  ...
  const [chartType, setChartType] = useState<"total" | "bridged" | "both">(metric)
  ...
  const getChartTitle = (): string => {
    ...
    switch (chartType) {
      ...
      case "both":
        return "Stablecoin Supply on Avalanche"
      default:
  ...
  const createChartData = () => {
    ...
    if (chartType === "bridged" || chartType === "both") {
      // Add bridged traces
      traces.push({
        x: stablecoinData.map(d => d.date),
        ...
        name: 'Total Bridged',
        line: {
          color: chartType === "both" ? '#FF9B9B' : '#E84142',
          width: 3,
          dash: chartType === "both" ? 'dot' : 'solid'
        }
      })
  ...
  const handleChartTypeChange = (type: "total" | "bridged" | "both") => {
    setChartType(type)
  }
